...
output "site_recovery_replication_recovery_plans_azure_to_azure_settings" {
  ...
  value       = { for k, v in azurerm_site_recovery_replication_recovery_plan.site_recovery_replication_recovery_plans : k => one(v.azure_to_azure_settings) if v.azure_to_azure_settings != null && length(v.azure_to_azure_settings) > 0 }
}
...
output "site_recovery_replication_recovery_plans_failover_recovery_group" {
  ...
  value       = { for k, v in azurerm_site_recovery_replication_recovery_plan.site_recovery_replication_recovery_plans : k => one(v.failover_recovery_group) if v.failover_recovery_group != null && length(v.failover_recovery_group) > 0 }
}
...
output "site_recovery_replication_recovery_plans_shutdown_recovery_group" {
  ...
  value       = { for k, v in azurerm_site_recovery_replication_recovery_plan.site_recovery_replication_recovery_plans : k => one(v.shutdown_recovery_group) if v.shutdown_recovery_group != null && length(v.shutdown_recovery_group) > 0 }
}
